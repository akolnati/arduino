...
function setup() {
  ...
  const fmOutButton = createButton("FM OUT");
  fmOutButton.mousePressed(ForwardMotorOut);
  fmOutButton.position(15, 60);

  const fmInButton = createButton("FM IN");
  fmInButton.mousePressed(ForwardMotorIn);
  fmInButton.position(fmOutButton.x + fmOutButton.width + 10, 60);

  const fmStopButton = createButton("FM STOP");
  fmStopButton.mousePressed(ForwardMotorStop);
  fmStopButton.position(fmInButton.x + fmInButton.width + 10, 60);
